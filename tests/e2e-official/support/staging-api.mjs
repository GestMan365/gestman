...
export const marker = "QA-3D-DESIGN-";
export const fixtureFile = path.join(env.root, "supabase", ".temp", "official-e2e-fixture.json");

export const fixture = Object.freeze({
  slugA: "qa-3d-design-a",
  slugB: "qa-3d-design-b",
  convertedSlug: "qa-3d-design-converted",
  ...
  requestEmail: "qa-3d-design-request@example.invalid",
...
export async function cleanupQa() {
  const companyResult = await rows("gm_companies", "select=id,slug&slug=like.qa-3d-design-*");
  ...
  await api("/rest/v1/gm_companies?slug=like.qa-3d-design-*", {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
  ...
  for (const user of users) {
    ...
    if (
      ...
      || email.includes("qa3ddesign")
      || String(metadata.company_slug || "").startsWith("qa-3d-design-")
    ) {
...
export async function cleanupSummary() {
  const [companies, requests, profiles, objects] = await Promise.all([
    rows("gm_companies", "select=id&slug=like.qa-3d-design-*"),
  ...
  const users = Array.isArray(listed.payload?.users)
    ? listed.payload.users.filter((user) =>
      ...
      || String(user.email || "").toLowerCase().includes("qa3ddesign"))
    : [];
