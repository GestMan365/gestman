...
-- ---------------------------------------------------------------------------
--
-- Supabase owns the storage schema and storage.objects table. This baseline
-- creates only application policies when that managed table is available.
-- The private gestman-attachments bucket must be provisioned separately via
-- the Supabase Storage API/CLI; no storage metadata row is inserted here.
  ...
  IF to_regclass('storage.objects') IS NOT NULL THEN
    ...
    EXECUTE $policy$
      ...
      USING (
        ...
        AND public.gm_is_company_member(
